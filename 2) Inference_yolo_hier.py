...
def _prepare_convert(classes_old, old2new):
    """ prepare the convert
    Args:
        classes_old: a list of old clsses
        old2new: a dictionary that indicates the conversion from old to new set of classes
    """
    ...
    return classes_new, new2old, new2old_idx
...
def parse_options():
    ...
    parser.add_argument('--best_epoch', type=makelist, default='424, 497, 435, 488, 497, 499', help='best performing model indices')
...
def main(opt):
    ...
    for iep in range(len(opt.best_epoch)): # for each model
        ...
        if any([re.search("LUAD", x) for x in all_gts.keys()]):
            ...
            all_gts_ = deepcopy(all_gts)
            all_results_ = deepcopy(all_results)
        ...
        if opt.model_type == "non-hier":
            ...
            classes_new, new2old, new2old_idx = _prepare_convert(classes_old, old2new)
            
        elif opt.model_type == "hier":
            pass
        ...
        #     data_dict = data_dicts['LUAD'] if 'LUAD' in dset else data_dicts[dset]
            ...
            if convert:
                ...
                if opt.model_type == "hier":
                    data_dict.get_data2model_root(tree_cls_infer)
                    classes_old = list(data_dict.data2model_root.keys())
                    '''classes_old = list(data_dict.d2m_root.keys())'''
                    old2new     = data_dict.data2model_root
                    classes_new, new2old, new2old_idx = _prepare_convert(classes_old=classes_old, old2new=old2new)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser("Inference for YOLO-hier", parents=[parse_options()])
